...
MVE-20260404-09: Causal reasoning diversity effect.
Uses selected_tasks.jsonl from pre-test.

Usage:
  python run_nca_v9.py --condition 7b_homo
  python run_nca_v9.py --condition 7b_het
"""
...
TASK_FILE = RESULTS_DIR / "selected_tasks.jsonl"
...
Evaluate this causal reasoning statement. Think carefully about:
...
Independently verify this causal reasoning. Do NOT simply agree.
Think about causation vs correlation, necessity vs sufficiency.
{agree}
...
Critically evaluate both answers on causal reasoning.
...
def main():
    parser = argparse.ArgumentParser(description="MVE-09 causal reasoning NCA experiment")
    parser.add_argument("--condition", required=True, choices=["7b_homo", "7b_het"])
    args = parser.parse_args()

    models = CONDITIONS[args.condition]
    output_file = f"causal_{args.condition}.jsonl"
    ...
    with open(TASK_FILE, "r", encoding="utf-8") as f:
        for line in f:
            if line.strip():
                ...
                t["task_input"] = t["question"]
                tasks.append(t)

    print("=" * 60)
    print(f"MVE-09: causal_reasoning x {args.condition}")
    print(f"Models: {models}")
    print(f"Steps: {STEPS}, Agree: {AGREE}")
    print(f"Tasks: {len(tasks)} (selected from pre-test)")
