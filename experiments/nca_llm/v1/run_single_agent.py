...
MVE-20260404-01: Single-agent baseline experiment.
Each model independently evaluates world_consistency tasks.
No inter-agent communication. One shot per task.
"""
...
def model_to_filename(model: str) -> str:
    """Convert model name to safe filename. e.g. 'qwen2.5:7b' -> 'single_qwen'."""
    name = model.split(":")[0].split(".")[0]  # 'qwen2.5:7b' -> 'qwen2'
    # Special cases for clarity
    if "qwen" in name:
        return "single_qwen"
    elif "llama" in name:
        return "single_llama3"
    elif "mistral" in name:
        return "single_mistral"
    return f"single_{name}"
...
def main():
    parser = argparse.ArgumentParser(description="Single-agent baseline for MVE-20260404-01")
    ...
    results_path = RESULTS_DIR / f"{filename}.jsonl"
