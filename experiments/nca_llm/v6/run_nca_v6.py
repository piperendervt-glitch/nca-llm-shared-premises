...
MVE-20260404-06: Diversity effect on logical reasoning tasks.

Usage:
  python run_nca_v6.py --condition 7b_homo
  python run_nca_v6.py --condition 7b_het
"""
...
from logic_task_generator import generate_tasks

OLLAMA_URL = "http://localhost:11434/api/generate"
RESULTS_DIR = Path(__file__).resolve().parent.parent.parent.parent / "results" / "nca_llm" / "v6"
...
Analyze the logical relationship step by step.
Determine if the statement logically follows from the world rule.
Answer CORRECT if the statement is logically valid, INCORRECT if not.
{agree}
...
{{
  ...
  "reasoning": "Step-by-step logical analysis in 2-3 sentences"
}}"""
...
Independently verify this logical reasoning from scratch.
Do NOT simply agree - check the logic yourself.
Determine if the statement logically follows from the world rule.
{agree}
...
def main():
    parser = argparse.ArgumentParser(description="MVE-06 logic reasoning NCA experiment")
    parser.add_argument("--condition", required=True, choices=["7b_homo", "7b_het"])
    args = parser.parse_args()

    models = CONDITIONS[args.condition]
    output_file = f"logic_{args.condition}.jsonl"
    ...
    print("=" * 60)
    print(f"MVE-06: logic_reasoning x {args.condition}")
