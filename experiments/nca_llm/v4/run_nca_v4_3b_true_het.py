...
MVE-20260404-04: NCA-LLM 3b heterogeneous (3 different 3b-class models).
qwen2.5:3b (Solver) / llama3.2:3b (Verifier) / granite3.1-moe:3b (Critic)
"""
...
RESULTS_DIR = Path(__file__).resolve().parent.parent.parent.parent / "results" / "nca_llm" / "v4"
RESULTS_PATH = RESULTS_DIR / "nca_3b_true_het_results.jsonl"
...
def main():
    ...
    print("=" * 60)
    print("NCA-LLM v4: 3b true-heterogeneous")
    print(f"Models: {MODELS}")
    print(f"Steps: {STEPS}, Agree: {AGREE}, Aggregation: Counter majority")
    print(f"Output: {RESULTS_PATH}")
    print("=" * 60)

    raw_tasks = generate_world_consistency_tasks()
    tasks = [{"task_id": f"wc_{t.task_id:03d}", "task_set": "world_consistency",
    ...
    with open(RESULTS_PATH, mode, encoding="utf-8") as f_out:
        for task in remaining:
            ...
            if done % 10 == 0 or done == total:
                ...
                print(f"  [NCA-3b-true-het] {done}/{total} ({acc:.1f}% acc) last={elapsed:.1f}s")
