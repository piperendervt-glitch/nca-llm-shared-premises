...
MVE-05 supplement: run 37 extra math_middle tasks for split accuracy power.

Usage:
  python run_nca_v5_extra.py --condition 7b_homo
  python run_nca_v5_extra.py --condition 7b_het
"""
...
TASK_FILE = Path(__file__).resolve().parent / "extra_middle_tasks.jsonl"
...
def main():
    parser = argparse.ArgumentParser(description="MVE-05 extra math_middle experiment")
    parser.add_argument("--condition", required=True, choices=["7b_homo", "7b_het"])
    args = parser.parse_args()

    models = CONDITIONS[args.condition]
    output_file = f"math_mid_{args.condition}_extra.jsonl"
    ...
    with open(TASK_FILE, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line:
                t = json.loads(line)
                t["task_input"] = f"Rule: {t['world_rule']}\nStatement: {t['question']}"
                tasks.append(t)

    print("=" * 60)
    print(f"MVE-05 EXTRA: math_middle x {args.condition}")
    print(f"Models: {models}")
    print(f"Steps: {STEPS}, Agree: {AGREE}")
    print(f"Tasks: {len(tasks)} (extra)")
    ...
    tag = f"math_mid-{args.condition}-extra"
