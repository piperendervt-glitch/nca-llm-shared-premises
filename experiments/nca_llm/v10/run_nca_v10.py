...
MVE-20260405-01: External task verification phase.
NCA settings identical to v1.

Usage:
  python run_nca_v10.py --source grok --condition 7b_homo
  python run_nca_v10.py --source grok --condition 7b_het
  python run_nca_v10.py --source chatgpt --condition 7b_homo
  python run_nca_v10.py --source chatgpt --condition 7b_het
"""
...
from task_loader import load_external_tasks

OLLAMA_URL = "http://localhost:11434/api/generate"
RESULTS_DIR = Path(__file__).resolve().parent.parent.parent.parent / "results" / "nca_llm" / "v10"
...
Based on the given world rule, determine if the statement is CORRECT or INCORRECT.
The world rule defines how this particular world works - it may differ from reality.
Think step by step.
{agree}
...
Independently verify this answer. Do NOT simply agree - check the logic yourself.
Remember: the world rule defines this world's rules, which may differ from reality.
{agree}
...
Remember: the world rule defines this world's rules.
{agree}
...
def main():
    parser = argparse.ArgumentParser(description="MVE-20260405-01 external task verification")
    parser.add_argument("--source", required=True, choices=["grok", "chatgpt"])
    parser.add_argument("--condition", required=True, choices=["7b_homo", "7b_het"])
    args = parser.parse_args()

    models = CONDITIONS[args.condition]
    output_file = f"{args.source}_{args.condition}.jsonl"

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    results_path = RESULTS_DIR / output_file

    tasks = load_external_tasks(args.source)

    print("=" * 60)
    print(f"MVE-20260405-01: {args.source} x {args.condition}")
    ...
    tag = f"{args.source}-{args.condition}"
    ...
    with open(results_path, mode, encoding="utf-8") as f_out:
        for task in remaining:
            ...
            record = {
                ...
                "task_type": task.get("task_type", "world_consistency"),
